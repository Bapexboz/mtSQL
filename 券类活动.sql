...
where br.datekey between  '$begindatekey' and '$enddatekey' 
     ...
     and bt.partition_is_paid = 1) b 
on a.order_key=b.order_id  and  a.sale_platform=b.sale_platform  
left join 
...
LEFT JOIN  ba_hotel.dim_activity_trigger_info b2 ON  b1.activity_id = b2.activity_id
...
SELECT
  ...
  activity_name,
  creator_name
FROM  ba_hotel.fact_pda_send_code_record 
WHERE  datekey >= '20140425' -- 限制三年内发券
  AND  card_code != ''
  AND  card_code != '0'
)d  on CAST(a.card_code AS STRING) = CAST(d.card_code AS STRING)  and a.datekey=d.datekey
